...
void vkcomponent::ShaderEffect::ReflectLayout(VkDevice device, ReflectionOverrides* overrides, int overrideCount)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		if (ly.create_info.bindingCount > 0) {
			setHashes[i] = vkcomponent::HashDescriptorLayoutInfo(&ly.create_info);
...
void vkcomponent::ShaderDescriptorBinder::BindBuffer(const char* name, const VkDescriptorBufferInfo& bufferInfo)
{
	BindDynamicBuffer(name, -1, bufferInfo);
}


void vkcomponent::ShaderDescriptorBinder::BindDynamicBuffer(const char* name, uint32_t offset, const VkDescriptorBufferInfo& bufferInfo)
{
...
void vkcomponent::ShaderDescriptorBinder::ApplyBinds(VkCommandBuffer cmd)
{
	for (int i = 0; i < 2; i++) {
...
void vkcomponent::ShaderDescriptorBinder::BuildSets(VkDevice device, DescriptorAllocator& allocator)
{
	...
	for (BufferWriteDescriptor& w : bufferWrites) {
		...
		VkWriteDescriptorSet write = vkinit::WriteDescriptorBuffer(w.descriptorType, VK_NULL_HANDLE, &w.bufferInfo, w.dstBinding);
...
void vkcomponent::ShaderDescriptorBinder::SetShader(ShaderEffect* newShader)
{
...
vkcomponent::ShaderModule* vkcomponent::ShaderCache::GetShader(const std::string& path)
{
	...
	{	
		...
		bool result = vkcomponent::LoadShaderModule(path.c_str(), &newShader.module, *p_device);
		if (!result)
		{
